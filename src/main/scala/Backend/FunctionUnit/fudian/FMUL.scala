...
class FMUL(val expWidth: Int, val precision: Int) extends Module {
  ...
  /*fmul_s2.io.in := ShiftRegister(fmul_s1.io.out, 1, WireInit(new FMUL_s1_to_s2(expWidth, precision),
    _.special_case -> WireInit(ValidIO(new FMUL_special_info),
      _.valid -> false.B,
      _.bits -> WireInit(new FMUL_special_info,
        _.nan -> false.B,
        _.inf -> false.B,
        _.inv -> false.B,
        _.hasZero -> false.B)),
    _.early_overflow -> false.B,
    _.prod_sign -> false.B,
    _.shift_amt -> 0.UInt((expWidth + 1).W),
    _.exp_shifted -> 0.UInt((expWidth + 1).W),
